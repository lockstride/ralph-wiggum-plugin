...
@test "PROMPT.md mode writes user content verbatim (0.13.1 — no loop-extras)" {
  # 0.13.1: dynamic state (activity tail, etc.) was removed from the body —
  # it belongs in the framing prompt, not in the durable user-supplied body.
  cat > "$MOCK_WORKSPACE/PROMPT.md" <<'PROMPT'
# My custom prompt
Do the work.
PROMPT
  ...
  # Activity tail / "Recent activity" section is NOT appended (was a layer
  # violation — dynamic state belongs in build_prompt's framing).
  ! grep -q "Recent activity" "$effective"
}

@test "--prompt-file mode writes user content verbatim (0.13.1 — no loop-extras)" {
  cat > "$MOCK_WORKSPACE/custom.md" <<'PROMPT'
# Custom prompt
do stuff
PROMPT
