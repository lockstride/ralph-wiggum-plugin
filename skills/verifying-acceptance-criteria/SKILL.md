...
2. **Verify each requirement independently.** For each one:
   ...
   - **Gate (test/lint/build) is part of the contract** — run it via `gate-run.sh` under the `final` label. This keeps eval-loop gate output in `.ralph/gates/final-latest.{log,exit,cmd}` separate from the impl loop's `full-latest.*` artifacts.
