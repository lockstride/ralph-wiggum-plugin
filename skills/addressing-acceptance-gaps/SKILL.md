...
2. **For each `[ ]` gap, in your chosen order:**
   ...
   - If a gate wrapper is present, run the relevant gate after non-trivial changes via `gate-run.sh` under the `final` label (the eval loop's tier — keeps your output in `final-latest.*`, separate from the impl loop's `full-latest.*`). If the gate fails, fix the failure before moving to the next gap.
