...
2. **For each `[ ]` gap, in your chosen order:**
   ...
   - If a gate wrapper is present, run the relevant gate after non-trivial changes via `gate-run.sh` under an `eval-rework` label. If the gate fails, fix the failure before moving to the next gap.
