---
description: Run exactly one Ralph loop in a terminal (smoke test your prompt)
argument-hint: "[--cli claude|cursor-agent] [--prompt | --prompt-file <path> | --spec [name]]"
---

Run a single loop of the Ralph loop — useful for smoke-testing your prompt or spec before committing to a full AFK run.
...
After the loop finishes, review:
...
If the loop looks good, switch to `/ralph` for the full loop.
