...
- **Tier-pin validation (0.20.0):** the launcher refuses (exit 64, before the lock) a tier label handed anything but its `[gates]` pin — the label→command half of the tier lock, and the same comparison `_complete_allowed` makes at COMPLETE. It carries a third inline copy of `_load_gates_from_policy` (`_gate_load_gates`; `ralph-guard.sh` has the other as `_guard_load_gates`) because gate-run.sh sources nothing. Keep all three in sync, and keep the normalization identical to `_complete_allowed`'s — a stricter one here would reject gates that DO satisfy the bar.
